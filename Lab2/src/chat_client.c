...
void *keyboard_thread()
{
	...
	////////Initiate the USB keyboard////////
	    ...
	    while (1) {
	        ...
	        //Clear the buf for usb keyboard package receiving
	        memset(buf,'\0',sizeof(buf));
	        //read the usb package
	        read_size = usb_interrupt_read(handle, ep, (char *) buf, endpoint->wMaxPacketSize, 0);
...
void thread_create(void)
{
        ...
        if((temp = pthread_create(&thread[0], NULL, ethernet_thread, NULL)) != 0)       //comment2
                printf("Error: Ethernet thread initiation failed!\n");
        else
                printf("Ethernet thread initiation successful!\n");
        if((temp = pthread_create(&thread[1], NULL, keyboard_thread, NULL)) != 0)  //comment3
                printf("Keyboard thread initiation failed!");
        else
                printf("Keyboard thread initiation successful!\n");
}

//thread_wait will wait for all the thread to end
void thread_wait(void)
{
        /*wait for thread to end*/
        if(thread !=0) {                   //comment4
                pthread_join(thread[0],NULL);
                printf("Ethernet thread finished\n");
        }
        if(thread[1] !=0) {                //comment5
                pthread_join(thread[1],NULL);
                printf("Keyboard thread finished\n");
        }
}
